 ...
 *
 * Uses uid = userid = employee.user_id (e.g. 990).
 * This is the standard ZKTeco approach — the IN01 supports uid 1-65535.
 *
...
function device_push_user(array $device, array $employee): array
{
    ...
    try {
        ...
        $name = preg_replace('/[^\x20-\x7E]/', '', $name); // ASCII only for device
        ...
        return [true, "User '{$name}' (UID {$uid}) pushed to {$device['name']} ({$device['ip_address']}). Employee can now scan their finger."];
    } catch (\Throwable $e) {
...
function device_enroll_finger(array $device, array $employee, int $finger = 1): array
{
    ...
    try {
        ...
        $uid  = (int)$employee['user_id'];
        $name = trim(($employee['first_name'] ?? '') . ' ' . ($employee['last_name'] ?? ''));
        $name = preg_replace('/[^\x20-\x7E]/', '', $name); // ASCII only
        if (strlen($name) > 24) $name = substr($name, 0, 24);
        if ($name === '') $name = 'User' . $uid;
        ...
        return [true,
            ...
            "Ask the employee to scan their finger NOW."
        ];
