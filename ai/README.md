...
### Données ajoutées par SQL

Le script utilise uniquement les employés et les pointages présents dans MySQL.
Les pointages dont la colonne `raw` contient `SQL_AI_TRAINING` sont reconnus
comme données synthétiques et les résultats sont clairement signalés dans
l’interface. La commande `--demo` n’est plus utilisée.

Ouvrir ensuite :

```text
http://localhost/attendance_system/dashboard/ai_predictions.php
```
